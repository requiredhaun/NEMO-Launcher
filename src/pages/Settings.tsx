...
export function Settings() {
  ...
  return (
    <div>
      <h1 className="h-dot">SETTINGS</h1>
      <p className="sub">RAM всего: {info?.totalRamMB} MB · рекомендовано: {info?.recommendedRamMB} MB</p>
      <div className="grid" style={{ gridTemplateColumns: 'repeat(auto-fit,minmax(300px,1fr))' }}>
        <div className="card">
          <div style={{ fontFamily: 'var(--font-dot)', letterSpacing: 2 }}>MEMORY — {cfg.ramMB} MB</div>
        ...
        <div className="card">
          <div style={{ fontFamily: 'var(--font-dot)', letterSpacing: 2 }}>JVM PRESET</div>
        ...
        <div className="card">
          <div style={{ fontFamily: 'var(--font-dot)', letterSpacing: 2 }}>JAVA & WINDOW</div>
          ...
          <div className="row" style={{ marginTop: 8 }}>
            ...
            <label className="row" style={{ gap: 6 }}><input type="checkbox" checked={cfg.fullscreenGame} onChange={(e) => set({ fullscreenGame: e.target.checked })} /> FS</label>
          </div>
          <div className="row" style={{ marginTop: 10 }}>
            <button className="btn ghost" onClick={() => call('launch:openGameFolder', {})}>GAME FOLDER</button>
            <button className="btn ghost" onClick={() => call('gamedir:reset').then(load)}>RESET DIR</button>
          </div>
        </div>
      </div>
    </div>
